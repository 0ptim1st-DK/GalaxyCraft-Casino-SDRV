...
function sky.drawImage(x,y,path)
    ...
    local success, result = pcall(serial.unserialize, text)
    if not success or result == nil or type(result) ~= "table" then
        g.setForeground(0xFFD700)
        g.set(x, y, "OpenCasino")
        g.setBackground(back)
        g.setForeground(font)
    ...
    if type(image) ~= "table" or #image < 2 then
        g.setForeground(0xFFD700)
        g.set(x, y, "OpenCasino")
        g.setBackground(back)
        g.setForeground(font)
    ...
    for i = 1, rows do
        x = start_x
        local bg_row = image[i * 2 - 1]
        local fg_row = image[i * 2]
        
        if bg_row and fg_row and type(bg_row) == "table" and type(fg_row) == "table" then
            for j = 1, math.min(#bg_row, #fg_row) do
                g.setBackground(bg_row[j] or 0x000000)
                g.setForeground(fg_row[j] or 0xFFFFFF)
                g.set(x, y, "▄")
                x = x + 1
            end
        end
...
function sky.takeItem(nick, item, numb)
...
function sky.mathRound(roundIn , roundDig)
    local mul = math.pow(10, roundDig)
    return ( math.floor(( roundIn * mul) + 0.5)/mul)
end

function sky.swap(array, index1, index2)
    array[index1], array[index2] = array[index2], array[index1]
end

function sky.shake(array)
    local counter = #array
    while counter > 1 do
...
function sky.get(url, filename,x,y)
...
function sky.getHostTime(timezone)
...
function sky.word(x,y,text,ramka)
...
function sky.symbol(x,y,symbol,ramka)
...
function sky.read(settings)
    ...
    while true do
        ...
        local event, address, char, code, nick = event.pull(settings.blink and math.min(0, nextBlink - computer.uptime()))
